...
for i, target in enumerate(target_cols):
    ...
    model_filename = f'../models/meta_model_{target}.pkl'
    with open(model_filename, 'wb') as f:
        pickle.dump(meta_learner_final, f)
    print(f"  Saved meta-learner model to {model_filename}", flush=True)
    
    # Store metadata for this target
    model_metadata[target] = {
        'selected_features': per_target_features[target],
...
print(f'Successfully saved {len(target_cols)} meta-learner models to ../models/') 